...
class QuantumTerminalCharm(CharmBase):
    def __init__(self, *args):
        ...
        self.framework.observe(self.on.delete_cross_connect_action, self._on_delete_cross_connect)
    ...
    def _on_delete_cross_connect(self, event):
        service_id = event.params.get("service-id", "example-qservice-01")
        ...
        try:
            response = requests.delete(restconf_url, headers=headers, timeout=5)
            if response.status_code in [200, 202, 204]:
                event.set_results({"status": "SUCCESS", "message": f"Service {service_id} deleted"})
            else:
                event.fail(f"RESTCONF HTTP {response.status_code}: {response.text}")
        except Exception as e:
            event.fail(f"Connection to controller failed: {str(e)}")
