...
run_lifecycle_benchmark() {
    ...
    for ((i=1; i<=ITERATIONS; i++)); do
        ...
        if [ "$nb_proto" == "RESTCONF" ]; then
            t_conn=$(time_exec "curl -s -X POST '${RESTCONF_GW_URL}' -H 'Content-Type: application/json' -H 'X-Southbound-Target: ${sb_proto}' -d '{\"service-id\":\"qservice-m${mode_id}-i${i}\",\"target-node\":\"${TARGET_DEVICE}\",\"target-node-ip\":\"${TARGET_NODE_IP}\",\"ingress-port\":1,\"egress-port\":2,\"admin-state\":\"ENABLED\"}'")
        else
