...
run_lifecycle_benchmark() {
    ...
    for ((i=1; i<=ITERATIONS; i++)); do
        ...
        elif [ "$sb_proto" == "NETCONF" ]; then
            t_conn=$(time_exec "kubectl exec -n micro-onos sdn-adapter -- python3 /tmp/netconf-switching-client.py ${TARGET_NODE_IP} connect")
        elif [ "$sb_proto" == "gNOI" ]; then
            t_conn=$(time_exec "kubectl exec -n micro-onos sdn-adapter -- python3 -c \"import socket; socket.create_connection(('${TARGET_NODE_IP}', 50051), timeout=2)\"")
        else
            t_conn=$(time_exec "gnmic -a ${ONOS_GNMI_TARGET} --tls-cert /etc/onos/certs/tls.crt --tls-key /etc/onos/certs/tls.key --skip-verify --target ${TARGET_DEVICE} set --update '/interfaces/interface[name=eth1]/config/name:::string:::eth1' --update '/interfaces/interface[name=eth1]/config/description:::string:::qservice-m${mode_id}-${sb_proto}'")
        fi
        ...
        if [ "$nb_proto" == "RESTCONF" ]; then
            ...
            t_stat1=$(time_exec "kubectl exec -n micro-onos sdn-adapter -- python3 /tmp/netconf-switching-client.py ${TARGET_NODE_IP} status")
        else
        ...
        if [ "$nb_proto" == "RESTCONF" ]; then
            ...
            t_disc=$(time_exec "kubectl exec -n micro-onos sdn-adapter -- python3 /tmp/netconf-switching-client.py ${TARGET_NODE_IP} disconnect")
        else
        ...
        if [ "$nb_proto" == "RESTCONF" ]; then
            ...
            t_stat2=$(time_exec "kubectl exec -n micro-onos sdn-adapter -- python3 /tmp/netconf-switching-client.py ${TARGET_NODE_IP} status")
        else
